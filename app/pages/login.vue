...
async function requestOtp() {
  ...
  try {
    ...
    if (error.response && error.response._data && error.response._data.message) {
      errorMessages.value = Array.isArray(error.response._data.message) ? error.response._data.message : [error.response._data.message];
    } else {
    ...
    <v-main class="d-flex align-center justify-center">
      <v-card class="pa-6" width="450" flat>
        <v-card-title class="text-h5 text-center mb-4">
          <span class="text-primary">Maizu</span> - ورود / ثبت‌نام
        </v-card-title>
        <v-card-text>
          <!-- مرحله ۱: درخواست OTP -->
          <v-form v-if="currentStep === 1" @submit.prevent="requestOtp" >
            <p class="mb-4 text-center">برای ورود یا ثبت‌نام، شماره تلفن خود را وارد کنید.</p>
            <v-text-field 
            v-model="phoneNumber" label="شماره تلفن" :rules="phoneNumberRules" variant="outlined" required
              class="mb-4" prepend-inner-icon="mdi-phone" persistent-hint /> <!-- اصلاح: self-closing tag -->
            <v-btn 
          ...
          <v-form v-else-if="currentStep === 2" @submit.prevent="verifyOtp" >
            <p class="mb-4 text-center">کد تأیید 5 رقمی ارسال شده به {{ phoneNumber }} را وارد کنید.</p>
            <v-otp-input v-model="otpCode" :rules="otpRules" :length="5" variant="solo-filled" class="mb-4" @keydown.enter="verifyOtp" />
            <!-- Vuetify 3: v-otp-input یا v-text-field با maxlength -->
            <v-alert v-if="otpCode.length > 0 && otpCode.length !== 5" type="warning" density="compact" class="mb-4">
              کد تأیید باید 5 رقمی باشد.
            </v-alert>
            <v-btn 
            type="submit" color="primary" block :loading="isLoading"
              :disabled="isLoading || otpCode.length !== 5" size="large" class="mt-4">
              تأیید و ورود
            </v-btn>
            <v-btn 
            variant="text" block class="mt-2"
              :disabled="isLoading" @click="currentStep = 1; otpCode = ''; successMessage = null; errorMessages = []" >
              ویرایش شماره تلفن
            </v-btn>
          </v-form>
